...
const (
	...
	PredicateTimeAnnotation = "tencent.com/predicate-time"     // 调度器分配给节点的时间
	PredicateGPUIndexPrefix = "tencent.com/predicate-gpu-idx-" // 给定容器应该在哪个GPU设备上运行的idx号
...
// IsGPUEnabledNode Is the Node has GPU device
func IsGPUEnabledNode(node *v1.Node) bool {
	return GetCapacityOfNode(node, VCoreAnnotation) > 0
}

// GetCapacityOfNode Get the capacity of request resource of the Node
...
func GetPredicateIdxOfContainer(pod *v1.Pod, containerIndex int) ([]int, error) {
	...
	if !ok {
		return ret, fmt.Errorf("predicate index for container %d of pod %s not found", containerIndex, pod.UID)
	}
...
// GetGPUResourceOfContainer returns the limit size of GPU resource of given container
